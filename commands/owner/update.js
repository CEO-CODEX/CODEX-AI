...
function getCfg(bot) {
    ...
    const repoFull = vars.UPDATE_REPO || bot.config.UPDATE_REPO || process.env.UPDATE_REPO || process.env.GITHUB_REPOSITORY || '';
    const [owner, repo] = repoFull.includes('/') ? repoFull.split('/') : [
        vars.UPDATE_OWNER || bot.config.UPDATE_OWNER || process.env.UPDATE_OWNER,
        vars.UPDATE_NAME || vars.UPDATE_REPO_NAME || bot.config.UPDATE_NAME || bot.config.UPDATE_REPO_NAME || process.env.UPDATE_NAME || process.env.UPDATE_REPO_NAME
    ];

    return {
        owner,
        repo,
        branch: vars.UPDATE_BRANCH || bot.config.UPDATE_BRANCH || process.env.UPDATE_BRANCH || 'master',
        token: vars.UPDATE_TOKEN || bot.config.UPDATE_TOKEN || process.env.UPDATE_TOKEN || process.env.GITHUB_TOKEN || ''
    };
}

function assertCfg(cfg) {
    if (!cfg.owner || !cfg.repo) {
        throw new Error('Update repo is not configured. Set UPDATE_REPO=owner/repo or UPDATE_OWNER and UPDATE_NAME.');
    }
}
...
function isSkipped(file) {
    ...
    return SKIP.some(item => {
        ...
        return norm === skip || norm.startsWith(skip + '/') || norm.includes('/' + skip + '/');
    });
}
...
async function currentSha(cfg) {
    ...
    const commits = await reqJson(cfg, `https://api.github.com/repos/${cfg.owner}/${cfg.repo}/commits?sha=${encodeURIComponent(cfg.branch)}&per_page=2`);
...
async function latestSha(cfg) {
    const latest = await reqJson(cfg, `https://api.github.com/repos/${cfg.owner}/${cfg.repo}/commits/${encodeURIComponent(cfg.branch)}`);
    return latest;
}
...
async function checkUpdates(cfg) {
    ...
    const cmp = await reqJson(cfg, `https://api.github.com/repos/${cfg.owner}/${cfg.repo}/compare/${curr}...${latest.sha}`);
    return {
        upToDate: false,
        curr: curr.slice(0, 8),
        latest: latest.sha.slice(0, 8),
...
async function applyUpdate(cfg) {
    ...
    const dependencyChanged = [...updated, ...removed].some(f => /(^|\/)package(-lock)?\.json$/.test(f));
    if (dependencyChanged && !failed.length) {
        await execPromise('npm install', { cwd: ROOT, timeout: 180000, maxBuffer: 1024 * 1024 });
    }

    const changed = [...updated, ...removed];
    const commandOnly = changed.length > 0 && changed.every(f => f.startsWith('commands/'));
    const needRestart = changed.some(f =>
        f === 'index.js' ||
        f === 'app.js' ||
        f === 'package.json' ||
        f === 'package-lock.json' ||
        ...
        try {
            if (!sub || sub === 'check') {
                ...
                const commits = result.commits
                    .slice(0, 10)
                    .map((c, i) => `${i + 1}. [${c.hash}] ${c.msg}${c.date ? ` (${c.date})` : ''}`)
                    .join('\n');
                const more = result.totalCommits > 10 ? `\n... and ${result.totalCommits - 10} more commits` : '';
