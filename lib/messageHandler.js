...
*/
      ...
      if (m.isGroup) {
        const isPrivileged = isOwner || this.bot.permission.isMod(sender, m._participantRaw);
        console.log('[antiSystems debug] outer gate:', { sender, isOwner, isPrivileged, willRunCheckAll: !isPrivileged });
        if (!isPrivileged) {
          console.log('[antiSystems debug] calling checkAll now...');
          let blocked = false;
          try {
            blocked = await this.bot.antiSystems.checkAll(m);
          } catch (err) {
            console.error('[antiSystems debug] checkAll call itself THREW at the call site:', err);
          }
          console.log('[antiSystems debug] checkAll returned:', blocked);
          if (blocked) return;
        }
      }
      ...
      // This used to be two blocks: an early "just go silent" check here
      // (using a stale direct-file read, keyed differently than muteStore)
      // that `return`d before ever reaching the real deletion logic further
      // down in the handler — so muted users' messages were never actually
      // deleted, only ignored by the bot. Matching SUKUNA_MD's actual
      // behaviour ("their messages will be auto-deleted until the mute is
      // lifted"), the real deletion now runs right here, before anything
      // else (auto-reply, commands, stickers) processes the message.
      if (m.isGroup && !m.key.fromMe && !isOwner) {
        try {
          const muteStore = require('./muteStore');
          const muteEntry = muteStore.getMute(sender);
          if (muteEntry) {
            if (!muteEntry.stickersOnly) {
              // Fully muted — delete ALL messages
              await this.bot.sock.sendMessage(jid, { delete: m.key }).catch(() => {});
              return;
            } else if (m.type === 'stickerMessage') {
              // Sticker-muted — delete ONLY stickers, text still works
              await this.bot.sock.sendMessage(jid, { delete: m.key }).catch(() => {});
              return;
            }
          }
      ...
      // BUG (was): this only ran deep inside the "matched a real command"
      // branch further down — so it fired only after a command name had
      // already been validated, never for plain chats, and there was no way
      // to show typing/recording for every incoming message. Moved here so
      // it runs for every message, with the same off/cmd/all modes as the
      // reference engine:
      //   off → never
      //   all → every message
      //   cmd → only messages that start with the bot's prefix
      // AUTO_TYPING/AUTO_RECORDING also stay backward compatible with the
      // old boolean true/false written by .autotyping / .autorecording
      // (true is treated as "cmd", matching their original "before commands"
      // description).
      if (!m.key.fromMe) {
        try {
          let _vars = {};
          try {
            _vars = JSON.parse(require("fs").readFileSync("./database/variables.json", "utf8"));
          } catch {}
          const ctCfg = this.bot.config.cmdTyping || {};
          const crecCfg = this.bot.config.cmdRecording || {};
          const _normMode = (raw, cfgEnabled) => {
            if (raw === true || raw === "true") return "cmd";
            if (raw === false || raw === "false" || raw === undefined) return cfgEnabled ? "cmd" : "off";
            return String(raw).toLowerCase(); // 'off' | 'cmd' | 'all'
          };
          const typingMode = _normMode(_vars.AUTO_TYPING, ctCfg.enabled);
          const recordingMode = _normMode(_vars.AUTO_RECORDING, crecCfg.enabled);
          const startsWithPrefix = !!text && text.startsWith(this.bot.prefix);

          const wantTyping = typingMode === "all" || (typingMode === "cmd" && startsWithPrefix);
          const wantRecording = recordingMode === "all" || (recordingMode === "cmd" && startsWithPrefix);
          ...
          m._presenceShown = wantTyping || wantRecording;
        } catch {}
      }
      ...
      try {
        // Typing/recording is now handled once, early, for every message
        // (see AUTO TYPING / AUTO RECORDING block above) — no longer
        // duplicated here per-command.
