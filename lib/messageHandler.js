...
class MessageHandler {
  ...
  _commandReaction(command) {
    const category = String(command?.category || '').toLowerCase();
    if (category.includes('admin') || category.includes('owner')) return '🔐';
    if (category.includes('group')) return '👥';
    if (category.includes('media')) return '🖼️';
    if (category.includes('download')) return '⬇️';
    if (category.includes('ai')) return '🧠';
    if (category.includes('search')) return '🔎';
    if (category.includes('game') || category.includes('gta')) return '🎮';
    if (category.includes('tools') || category.includes('bot')) return '⚙️';
    return '⏳';
  }
  ...
  async _run(command, m, args, cmdName) {
    ...
    const config = this.bot.config.cmdReact || {};
    const enabled = config.enabled === true;
    const key = m.key;
    if (enabled && key && this.bot.sock?.sendMessage) {
      await this.bot.sock.sendMessage(m.chat, {
        react: { text: command.reactEmoji || this._commandReaction(command), key },
      }).catch(() => {});
    }
    try {
      return await command.execute(this.bot, commandMessage, this._argv(commandMessage, args), cmdName);
    } finally {
      if (enabled && key && this.bot.sock?.sendMessage) {
        await this.bot.sock.sendMessage(m.chat, { react: { text: '', key } }).catch(() => {});
      }
    }
  }
