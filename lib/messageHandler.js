...
class MessageHandler {
  ...
  _commandContext() {
    ...
    context.sock = socket;
    for (const method of ['sendMessage', 'groupMetadata', 'groupParticipantsUpdate', 'presenceSubscribe', 'readMessages', 'relayMessage']) {
      ...
      if (m.isGroup) {
        try {
          ...
          if (_botToggle[jid]?.enabled === false) {
            ...
            if (!/^bot\s+on\b/i.test(text.trim())) return;
          }
        } catch {}
      }
...
*/
      ...
      /* MENTION-REACT REMOVED: normal mentions and owner mention commands remain available. */
      if (false && !m.fromMe) {
        try {
