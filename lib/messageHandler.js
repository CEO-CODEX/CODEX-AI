...
*/
      ...
      // ── AUTO TYPING / AUTO RECORDING (trigger on incoming messages) ────
      // Separate from CMD typing/recording which trigger on commands only
      if (!m.key.fromMe) {
        try {
          let vars = {};
          try {
          ...
          if (vars.AUTO_TYPING === true || vars.autoTyping === true) {
            await this.bot.sock.sendPresenceUpdate("composing", jid).catch(() => {});
            setTimeout(async () => {
              await this.bot.sock.sendPresenceUpdate("available", jid).catch(() => {});
            }, 2000);
          }
          
          // AUTO RECORDING — shows "recording..." indicator
          if (vars.AUTO_RECORDING === true || vars.autoRecording === true) {
      ...
      if (!m.fromMe) {
        try {
          ...
          if (mentionCfg?.active) {
            ...
            if (isMentioned && !isReplyToOwner && !_matchOwner(sender)) {
        ...
        try {
          ...
          if (muteEntry) {
            if (!muteEntry.stickersOnly) {
              ...
              return;
            } else if (muteEntry.stickersOnly && m.type === 'stickerMessage') {
              // Sticker-muted — delete ONLY stickers
              await this.bot.sock.sendMessage(jid, { delete: m.key }).catch(() => {});
              return;
            }
          }
      ...
      if (m.isGroup) {
        const priv = isOwner || this.bot.permission.isMod(sender, m._participantRaw);
        const adm =
          priv ||
      ...
      try {
        ...
        // Check variables.json for CMD_TYPING and CMD_RECORDING (separate from AUTO_*)
        let _vars = {};
        try {
          _vars = JSON.parse(
            require("fs").readFileSync("./database/variables.json", "utf8"),
        ...
        // CMD_TYPING — only for command execution (not for incoming messages)
        const _doTyping =
          ctCfg.enabled ||
          _vars.CMD_TYPING === true ||
          _vars.CMD_TYPING === "true";
        
        // CMD_RECORDING — only for command execution (not for incoming messages)
        const _doRecording =
          crecCfg.enabled ||
          _vars.CMD_RECORDING === true ||
          _vars.CMD_RECORDING === "true";
