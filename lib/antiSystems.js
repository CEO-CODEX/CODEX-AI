...
class AntiSystems {
    ...
    async checkAll(m) {
        ...
        //
        // BUG (was): this matched against `userId` (= m.sender), but
        // messageHandler.smsg() already runs `.replace(/:[0-9]+@/, "@")` on
        // the sender before antiSystems ever sees it — so the device suffix
        // is always gone by the time it gets here and the regex could never
        // match anything. The suffix is still intact on `m._participantRaw`
        // (set in smsg() specifically to preserve it), so check that instead.
        const abSettings = this._settings('./database/antibot.json', groupId);
        if (abSettings.enabled) {
            const rawParticipant = m._participantRaw || '';
            const mdMatch = String(rawParticipant).match(/^(\d+):(\d+)@s\.whatsapp\.net$/);
