...
class AntiSystems {
    ...
    async checkAll(m) {
        ...
        // Anti-GC-Status (per-group, off by default) — separate toggle from
        // Anti-Group-Mention above; shares the same forwarded-status detection
        // but its own database/action/autoDelete settings.
        ...
        if (agcsSettings.enabled) {
            ...
            if (isStatusMention) {
                if (agcsSettings.autoDelete !== false) await this._tryDelete(m);
                return await this._doAction(groupId, userId, agcsSettings, 'Group status mentions are not allowed', m);
            }
        }
