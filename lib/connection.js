    ...
    for (const msg of messages) {
      try {
        ...
        if (msg.key.remoteJid === "status@broadcast") {
          ...
          if (statusView) {
            await bot.sock.readMessages([{ ...msg.key, remoteJid: 'status@broadcast', participant: posterJid }]).catch(() => {});
          }
          if (statusReact && posterJid && posterJid !== bot.sock.user?.id) {
            await new Promise((resolve) => setTimeout(resolve, 2000));
            await bot.sock.sendMessage(posterJid, {
              react: { text: statusVars.STATUS_EMOJI || bot.config.statusReact.emoji || '💚', key: msg.key },
            }).catch(() => {});
          }
          continue;
        }
