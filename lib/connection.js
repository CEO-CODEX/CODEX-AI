...
async function startConnection(bot) {
  ...
  bot.sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      // Do not send automatic channel follows or startup messages here.
      // Keeping the socket quiet after pairing avoids unsolicited traffic and
      // reduces the chance of WhatsApp treating the account as spam.
    }
  });
