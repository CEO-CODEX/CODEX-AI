...
async function startConnection(bot) {
  ...
  bot.sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      // ── Auto-follow channel — fires once per process lifetime ─────────────
      // When someone deploys this bot, they'll automatically follow the
      // CODEX VERIFIED channel. count:'once' means no duplicate follows.
        ...
        setTimeout(async () => {
          try {
            ...
            await bot.sock.sendMessage(ownerJid, {
              followMe:  true,
              channelId: '120363425299923811@newsletter',
              count:     'once',
            }).catch(() => {});
          } catch {}
        }, 5000);
  ...
  bot.sock.ev.on("messages.upsert", async ({ type, messages }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // Ignore WhatsApp status traffic entirely; this bot does not view, react to, or save statuses.
