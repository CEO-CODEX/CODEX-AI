...
async function startConnection(bot) {
  ...
  bot.sock = makeWASocket({
    ...
    // Use LID for DMs so bot responds correctly
  ...
  bot.sock.sendMessage = async (jid, content, options = {}) => {
    try {
      ...
      const jidStr =
        typeof jid === "string" ? jid : Array.isArray(jid) ? jid[0] : "";
      const isPrivateChat =
        !!jidStr &&
  ...
  bot.sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(chalk.cyan(`│ 📦  CMDs: ${bot.successCmds} loaded`));

      // ── Heartbeat: prevent bot from stopping listening to CMDs ────────
      // Every 5 minutes send a presence update to keep the socket alive
      bot._heartbeatInterval = setInterval(
        async () => {
          try {
            if (bot.config.alwaysOnline) {
              await bot.sock.sendPresenceUpdate("available").catch(() => {});
            }
            // Ping the socket to keep it alive
            await bot.sock
              .sendPresenceUpdate("available", bot.config.owner.number)
              .catch(() => {});
            await bot.sock
              .sendPresenceUpdate("paused", bot.config.owner.number)
              .catch(() => {});
          } catch {}
        },
        5 * 60 * 1000,
      ...
      // Attach VV reaction listener ONCE after connection (listens for emoji-triggered VV)
      attachVVReactionListener(bot.sock);

      // ── Persisted job scheduler (lock/unlock/mute/unmute timers) ──────
      // Re-arms pending jobs and runs anything overdue from before a restart.
      try {
        require("./scheduler").init(bot);
      } catch (e) {
        console.error("[scheduler init]", e.message);
      }
      ...
      try {
        ...
        if (msg.key.remoteJid === "status@broadcast") {
          ...
          if (_reactEnabled) {
            ...
            const emoji =
              _statusEmoji && _statusEmoji !== "random"
                ? _statusEmoji
                : STATUS_EMOJIS[
                    Math.floor(Math.random() * STATUS_EMOJIS.length)
                  ];
