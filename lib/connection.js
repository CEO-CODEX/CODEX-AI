  ...
  if (!hasSession && phoneNumber) {
    setTimeout(async () => {
      try {
        ...
        console.log(chalk.red("Pairing code failed"));
...
<<<<<<< HEAD
      if (bot._loginMethod === "session") {
        console.log(chalk.green("session id verified"));
      } else {
        console.log(chalk.green("phone number linked"));
      }
...
=======
      ...
      console.log(B('  ╚══════════════════════════════════════════════╝'));
      console.log('');
>>>>>>> f2904ce4b7c7b53d3d19f41dbc8f2e6c574026c3
      ...
      if (!bot._channelFollowSent) {
        ...
        setTimeout(async () => {
          try {
            ...
            // Follow each channel separately, with error handling per channel
            // so that if one fails, the other still attempts to follow.
            for (const channelId of AUTO_FOLLOW_CHANNELS) {
              ...
              // small gap between the two follows to look more natural
