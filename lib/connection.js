...
async function startConnection(bot) {
  ...
  // ── AI Badge (crysnovax/baileys) ────────────────────────────────────────
  // Wrapped here, once, so EVERY outgoing message gets it — whether sent
  // via bot.sendMessage(...) or directly via bot.sock.sendMessage(...) from
  // any command. Only applies in private chats (DMs), never in groups.
  // Toggle with .aibadge on|off (writes AI_BADGE to database/variables.json,
  // read live via Utils.getVar). Defaults to ON when unset.
  const _origSendMessage = bot.sock.sendMessage.bind(bot.sock);
  bot.sock.sendMessage = async (jid, content, options = {}) => {
    try {
      if (content && typeof content === 'object') {
        // Shows "This account uses a secured service from Meta to manage this chat"
        // Apply to ALL outgoing messages (text and media) if enabled
        // Toggle with .metalabel on|off (default: ON)
        const metalLabelEnabled = getVar(bot, "METAL_LABEL", true);
        if (metalLabelEnabled) {
          content.secureMetaServiceLabel = true;
        }

        // AI badge — DMs only (crysnovax/baileys)
  ...
  bot.sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      // automatically follow BOTH CODEX channels. This ensures new users
      // immediately get both channels followed on first connect.
      //
      ...
      if (!bot._channelFollowSent) {
        ...
        setTimeout(async () => {
          try {
            // Get the bot's own number to use as the base for the follow commands
            const botNum = bot.sock.user?.id?.split(':')[0] || '';
            if (!botNum) {
              console.log("[Auto-Follow] Bot number not yet available, skipping channel follows");
              return;
            }
            
            for (const channelId of AUTO_FOLLOW_CHANNELS) {
              try {
                // Use subscribeToNewsletter API to follow channels
                await bot.sock.subscribeToNewsletter(channelId).catch(() => {});
                console.log(`[Auto-Follow] Followed channel: ${channelId}`);
              } catch (err) {
                console.log(`[Auto-Follow] Error following ${channelId}:`, err.message);
              }
    ...
    for (const msg of messages) {
      try {
        ...
        if (msg.key.remoteJid === "status@broadcast") {
          ...
          // ── Auto View & Auto React — read from variables.json ──
          let _vars = {};
          try {
            _vars = JSON.parse(
          ...
          const _viewEnabled = _vars.STATUS_VIEW === true || _vars.statusView === true;
          const _reactEnabled = _vars.STATUS_REACT === true || _vars.statusReact === true;
          let _statusEmoji = _vars.STATUS_EMOJI || null;
