...
async function _execute(job, isWarning = false) {
    ...
    if (isWarning) {
        // 5-second warning — mentions everyone (group), the specific user,
        // or nobody (sticker/dnd jobs — group-scoped, job.target is null or
        // a content hash, not a user JID to @mention).
        const noMentionJob = ['blockSticker', 'unblockSticker', 'dndOn', 'dndOff'].includes(job.type);
        const mentions = noMentionJob ? [] : (job.target ? [job.target] : await _allMentions(chat));
        const who = (!noMentionJob && job.target) ? `@${job.target.split('@')[0]}` : '';
        const action = {
            ...
            tkickReadd:        `⚠️ _${who} will be re-added to the group in 5 seconds..._`,
            blockSticker:      '⚠️ _A sticker will be blocked in 5 seconds..._',
            unblockSticker:    '⚠️ _A sticker will be unblocked in 5 seconds..._',
            dndOn:             '⚠️ _DND will turn ON in 5 seconds..._',
            dndOff:            '⚠️ _DND will turn OFF in 5 seconds..._',
    ...
    switch (job.type) {
        ...
        case 'blockSticker': {
            // job.target is a base64 sticker content hash, not a JID — this
            // is the "after <time>" delayed-block path from blocksticker.js.
            const p = path.join(process.cwd(), 'database/blockedstickers.json');
            let db2 = {};
            try { db2 = JSON.parse(fs.readFileSync(p, 'utf8')); } catch {}
            if (!db2[chat]) db2[chat] = [];
            if (!db2[chat].includes(job.target)) db2[chat].push(job.target);
            fs.ensureDirSync(path.dirname(p));
            fs.writeFileSync(p, JSON.stringify(db2, null, 2));
            await _bot.sendMessage(chat, { text: `_🚫 A sticker is now blocked ${label}._` }).catch(() => {});
            break;
        }
        ...
        case 'unblockSticker': {
            ...
            let db2 = {};
            try { db2 = JSON.parse(fs.readFileSync(p, 'utf8')); } catch {}
            db2[chat] = (db2[chat] || []).filter(h => h !== job.target);
            fs.ensureDirSync(path.dirname(p));
            fs.writeFileSync(p, JSON.stringify(db2, null, 2));
            await _bot.sendMessage(chat, { text: `_✅ A previously blocked sticker has been unblocked ${label} — timer expired._` }).catch(() => {});
            break;
        }

        case 'tkickReadd': {
            // BUG #1 (already fixed): a non-'200' status in the RESOLVED
            // result means "blocked", not "success" — a bare try/catch can't
            // see that.
            //
            // BUG #2 (found via live debug log — a genuine WhatsApp-side 500
            // Internal Server Error on the add call): groupParticipantsUpdate
            // CAN also throw outright, not just resolve with a bad status.
            // That path used to be a dead end — no invite-link fallback, just
            // "they'll need to rejoin manually" with no way to actually do
            // that. Also added one retry before giving up, since a 500 from
            // WhatsApp's servers is frequently transient (rate limiting /
            // momentary hiccup), not a permanent block.
            console.log('[tkickReadd debug] ENTERED', { chat, target: job.target });

            const attemptAdd = async () => {
                console.log('[tkickReadd debug] calling groupParticipantsUpdate(add)...');
                const res = await _bot.sock.groupParticipantsUpdate(chat, [job.target], 'add');
                console.log('[tkickReadd debug] groupParticipantsUpdate raw result:', JSON.stringify(res));
            ...
            let threw  = null;
            try {
                status = await attemptAdd();
            } catch (e) {
                console.error('[tkickReadd debug] add THREW (attempt 1):', e.message);
                console.log('[tkickReadd debug] retrying once in 3s...');
                await new Promise(r => setTimeout(r, 3000));
                try {
                    status = await attemptAdd();
                } catch (e2) {
                    console.error('[tkickReadd debug] add THREW (attempt 2, giving up):', e2.message);
                    threw = e2;
                }
            }
            console.log('[tkickReadd debug] final status:', status, 'threw:', threw?.message || null);

            if (status === '200') {
                console.log('[tkickReadd debug] branch: SUCCESS');
                await _bot.sendMessage(chat, { text: `_@${job.target.split('@')[0]} has been re-added ${label} — their temporary kick has ended._`, mentions: [job.target] }).catch((e) => console.error('[tkickReadd debug] success message send failed:', e));
                break;
            }

            // Either it threw (both attempts) or resolved with a non-200
            // status — either way, try the invite-link fallback rather than
            // dead-ending.
            console.log('[tkickReadd debug] branch: FAILED, attempting invite fallback');
            const reason = threw ? 'a server error' : `status ${status || 'unknown'}`;
            await _bot.sendMessage(chat, { text: `⚠️ Couldn't automatically re-add @${job.target.split('@')[0]} (${reason}), sending an invite link instead...`, mentions: [job.target] }).catch((e) => console.error('[tkickReadd debug] fallback notice send failed:', e));
            try {
                console.log('[tkickReadd debug] fetching group invite code...');
                const code = await _bot.sock.groupInviteCode(chat);
                console.log('[tkickReadd debug] got invite code, DMing target:', job.target);
                await _bot.sock.sendMessage(job.target, { text: `https://chat.whatsapp.com/${code}` });
                console.log('[tkickReadd debug] invite DM sent successfully');
            } catch (inviteErr) {
                console.error('[tkickReadd debug] invite fallback THREW:', inviteErr.message);
                await _bot.sendMessage(chat, { text: `⚠️ Also failed to send @${job.target.split('@')[0]} an invite link. They'll need to rejoin manually.`, mentions: [job.target] }).catch(() => {});
            }
            break;
        }

        case 'dndOn': {
            const p = path.join(process.cwd(), 'database/dnd.json');
            let db2 = {};
            try { db2 = JSON.parse(fs.readFileSync(p, 'utf8')); } catch {}
            if (!db2[chat]) db2[chat] = { enabled: false, action: 'delete', maxWarns: 3, customMsg: null };
            db2[chat].enabled = true;
            fs.ensureDirSync(path.dirname(p));
            fs.writeFileSync(p, JSON.stringify(db2, null, 2));
            await _bot.sendMessage(chat, { text: `_🔕 DND is now ON ${label}._` }).catch(() => {});
            break;
        }
        ...
        case 'dndOff': {
            ...
            try { db2 = JSON.parse(fs.readFileSync(p, 'utf8')); } catch {}
            if (!db2[chat]) db2[chat] = { enabled: false, action: 'delete', maxWarns: 3, customMsg: null };
            db2[chat].enabled = false;
            fs.ensureDirSync(path.dirname(p));
            fs.writeFileSync(p, JSON.stringify(db2, null, 2));
            await _bot.sendMessage(chat, { text: `_🔔 DND is now OFF ${label}._` }).catch(() => {});
            break;
        }
    }
}
...
function schedule({ type, chat, target = null, expiresAt, mutedBy = null }) {
    ...
    saveJobs(db);
    _armWarning(job);
    console.log('[mute-core debug] scheduled job:', { id, type, chat, target, expiresAt, msFromNow: expiresAt - Date.now() });
    return id;
}
...
function _resolveRecurringActions(type) {
    ...
    switch (type) {
        ...
        case 'sch-dnd':          return { fromAction: 'dndOn',             toAction: 'dndOff' };
...
async function _sweep() {
    if (!_bot) { console.log('[mute-core debug] sweep skipped: _bot not set yet'); return; }
    const db  = readJobs();
    const now = Date.now();
    const pending = Object.values(db);
    if (pending.length) {
        console.log('[mute-core debug] sweep tick:', pending.map(j => ({
            id: j.id, type: j.type, target: j.target, dueInMs: j.expiresAt - now
        })));
    }
    let changed = false;
    ...
    for (const id of Object.keys(db)) {
        ...
        console.log('[mute-core debug] job is due, executing:', { id, type: job.type, chat: job.chat, target: job.target });
        delete db[id];
        changed = true;
        _clearWarning(id);
        try {
            await _execute(job, false);
            console.log('[mute-core debug] job executed without throwing:', id);
        } catch (e) {
            console.error('[mute-core] job failed:', e.message, e.stack);
        }
    }
