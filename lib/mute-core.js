...
async function _execute(job, isWarning = false) {
    ...
    if (isWarning) {
        ...
        // or nobody (sticker jobs — job.target is a content hash, not a JID,
        // so there's no one to @mention).
        const isStickerJob = job.type === 'blockSticker' || job.type === 'unblockSticker';
        const mentions = isStickerJob ? [] : (job.target ? [job.target] : await _allMentions(chat));
        const who = (!isStickerJob && job.target) ? `@${job.target.split('@')[0]}` : '';
        const action = {
