...
𝗖𝗢𝗗𝗘𝗫 𝐀𝐈 𝐕𝟑`;
  ...
  // Read from groupEvents.json (CODEX pattern: one file, all group event config)
    ...
    if (action === 'add') {
      // Default to OFF unless a group has explicitly opted in (per-group
      // welcomeEnabled) or the global config explicitly turns it on.
      // Was: `cfg.welcomeEnabled ?? (this.config.welcome !== false)` —
      // that only stayed off if config.welcome was exactly `false`; a
      // missing/undefined key there would silently default to ON.
      const enabled = cfg.welcomeEnabled ?? (this.config.welcome === true);
      if (!enabled) return;
...
║╰────────────────────◆   
 ...
 ═══════════════════❒`;
      ...
      // Same off-by-default guarantee as welcome, see note above.
      const enabled = cfg.goodbyeEnabled ?? (this.config.goodbye === true);
      if (!enabled) return;
